 ...
 */
public class WormholeConfig implements SettingsHolder {
    
    // Property definitions with default values and comments
        @Comment("Number of seconds after a gate is activated, but before dialing before timing out.")
        public static final Property<Integer> TIMEOUT_ACTIVATE = PropertyInitializer.newProperty("timeouts.activate", 30);
    
        @Comment("Number of seconds after a gate is dialed before automatically shutdown. With 0 timeout a gate won't shutdown until something goes through the gate.")
        public static final Property<Integer> TIMEOUT_SHUTDOWN = PropertyInitializer.newProperty("timeouts.shutdown", 38);
    
        @Comment("Enable build count restrictions. Requires complex permissions.")
        ...
        @Comment("Total number of stargates a member of build restriction group one can build.")
        ...
        @Comment("Total number of stargates a member of build restriction group two can build.")
        ...
        @Comment("Total number of stargates a member of build restriction group three can build.")
        ...
        @Comment("Default permission level for players if built-in permissions are used. Values: WORMHOLE_USE_PERMISSION, WORMHOLE_BUILD_PERMISSION, WORMHOLE_CONFIG_PERMISSION, WORMHOLE_ADMIN_PERMISSION")
        ...
        @Comment("If true, external permissions plugins will be ignored even if available.")
        ...
        @Comment("Use simplified permission nodes (wormhole.simple.*) instead of the full permission set.")
        public static final Property<Boolean> SIMPLE_PERMISSIONS = PropertyInitializer.newProperty("permissions.simple_mode", false);
    
        @Comment("If true, wormhole.use permission is required to teleport. If false, only needed to activate gates.")
        ...
        @Comment("If true, Help plugin integration will be disabled.")
        public static final Property<Boolean> HELP_SUPPORT_DISABLE = PropertyInitializer.newProperty("help.disable", false);
    
        @Comment("If true, Wormhole X-Treme Worlds will be used for world management.")
        public static final Property<Boolean> WORLDS_SUPPORT_ENABLED = PropertyInitializer.newProperty("worlds.enabled", false);
    
        @Comment("Logging level. Values: SEVERE, WARNING, INFO, CONFIG, FINE, FINER, FINEST")
        public static final Property<String> LOG_LEVEL = PropertyInitializer.newProperty("logging.level", "INFO");
    
    private final SettingsManager settingsManager;
    
    // No-args constructor required by ConfigMe reflection when creating configuration data.
    public WormholeConfig() {
        this.settingsManager = null; // Not used in the reflective instantiation for property collection.
    }
    ...
    this.settingsManager = SettingsManagerBuilder
        ...
        .configurationData(ConfigurationDataBuilder.createConfiguration(WormholeConfig.class))
